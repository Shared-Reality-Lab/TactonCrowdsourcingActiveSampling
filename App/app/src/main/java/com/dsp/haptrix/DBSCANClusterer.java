...
public class DBSCANClusterer<V> {
    ...
    private ArrayList<ArrayList> inputValues = null;

    /** index maintaining visited points */
    private HashSet<ArrayList> visitedPoints = new HashSet<ArrayList>();
     ...
     */
    public DBSCANClusterer(final ArrayList<ArrayList> inputValues, int minNumElements, double maxDistance, DistanceMetric<V> metric) throws DBSCANClusteringException {
     ...
     * @throws DBSCANClusteringException 
     */
    public void setInputValues(final ArrayList<ArrayList> collection) throws DBSCANClusteringException {
        if (collection == null) {
            throw new DBSCANClusteringException("DBSCAN: List of input values is null.");
        }
        this.inputValues = new ArrayList<ArrayList>(collection);
    }
     ...
     * @throws DBSCANClusteringException 
     */
    private ArrayList<ArrayList> getNeighbours(final ArrayList inputValue) throws DBSCANClusteringException {
        ArrayList<ArrayList> neighbours = new ArrayList<ArrayList>();
        for(int i=0; i<inputValues.size(); i++) {
            ArrayList candidate = inputValues.get(i);
     ...
     */
    private ArrayList<ArrayList> mergeRightToLeftCollection(final ArrayList<ArrayList> neighbours1,
            final ArrayList<ArrayList> neighbours2) {
        for (int i = 0; i < neighbours2.size(); i++) {
            ArrayList tempPt = neighbours2.get(i);
     ...
     * @throws DBSCANClusteringException 
     */
    public ArrayList<ArrayList> performClustering() throws DBSCANClusteringException {
        ...
        ArrayList<ArrayList> neighbours;
        int index = 0;

        while (inputValues.size() > index) {
            ArrayList p = inputValues.get(index);
            if (!visitedPoints.contains(p)) {
                ...
                if (neighbours.size() >= minimumNumberOfClusterMembers) {
                    ...
                    while (neighbours.size() > ind) {
                        ArrayList r = neighbours.get(ind);
                        if (!visitedPoints.contains(r)) {
                            visitedPoints.add(r);
                            ArrayList<ArrayList> individualNeighbours = getNeighbours(r);
                            if (individualNeighbours.size() >= minimumNumberOfClusterMembers) {//from here
